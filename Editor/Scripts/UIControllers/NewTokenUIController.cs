    ...
    public class NewTokenUIController : PopupUIController
    {
        ...
        private async void OnAddButtonClicked()
        {
            ...
            try
            {
                ...
                Error = null;
                var token = await PckgsApi.GetAccessTokenDetails(tokenInput);
                if (PckgsWindow.UpmConfigs.Data.Any(c => c.EndPoint.EndsWith(token.OrganizationSlug)))
                {
                    Error = $"Token for organization '{token.OrganizationName}' is already added. Only one token can be added for each organization.";
                    return;
                }
